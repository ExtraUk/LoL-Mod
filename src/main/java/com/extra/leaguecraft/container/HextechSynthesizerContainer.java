...
public class HextechSynthesizerContainer extends Container {
    ...
    public HextechSynthesizerContainer(int windowId, World world, BlockPos pos, PlayerInventory playerInventory, PlayerEntity player) {
        super(ModContainers.HEXTECH_SYNTHESIZER_CONTAINER.get(), windowId);
        ...
        if(tileEntity != null){
            tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                addSlot(new SlotItemHandler(h, 0, 16, 55));
                addSlot(new SlotItemHandler(h, 1, 34, 55));
                addSlot(new SlotItemHandler(h, 2, 34, 26));
                addSlot(new SlotItemHandler(h, 3, 126, 36));
            });
        }
    }
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(IWorldPosCallable.of(tileEntity.getWorld(), tileEntity.getPos()),
                playerIn, ModBlocks.HEXTECH_SYNTHESIZER.get());
    }
    ...
    public int getShimmerAmount(){
        return ((HextechSynthesizerTile) this.tileEntity).getShimmerAmount();
    }
